		...
		return function (done) {
			...
			request(thisRequestOptions, function (err, res, body) {
				...
				if (err) {
					...
					var error = new Error('Status ' + res.statusCode);
...
actions.getUserEvents = function (req, res) {
	// TODO-NG: user should be able to view global events on public github, too
	var r;
	if (process.env.GITHUB_ENTERPRISE_URL) {
		r = githubApi.events(req.session.authToken);
	} else {
		r = githubApi.userReceivedEvents(req.user, req.session.authToken);
	}
	multiPageFetch(r, DEFAULT_PAGES, function (err, receivedEvents) {
		if (err) {
			console.error(err);
			var status = err.statusCode || 500;
			return res.send(status);
